...
async function prepareImage(file: File) {
    ...
    if (file.size > 30 * IMAGE_MAX_BYTES) throw new Error(`${file.name}: 原图超过 30 MiB，请先自行缩小。`);
    const bytes = new Uint8Array(await file.arrayBuffer());
    let info;
    try { info = inspectImage(bytes); }
    ...
    try {
        ...
        if (!context) throw new Error(`${file.name}: 浏览器无法压缩图片。`);
        let factor = Math.min(1, 2560 / Math.max(bitmap.width, bitmap.height));
        for (let attempt = 0; attempt < 8; attempt++) {
            ...
            if (!blob || blob.type !== file.type) throw new Error(`${file.name}: 浏览器无法重新编码此图片，请换用支持 JPG 和 PNG 编码的浏览器。`);
            if (blob.size <= IMAGE_MAX_BYTES) return new File([blob], file.name.replace(/\.[^.]+$/, "") + (blob.type === "image/png" ? ".png" : ".jpg"), { type: blob.type });
            factor *= 0.8;
        }
        throw new Error(`${file.name}: 压缩后仍超过 1 MiB，请自行缩小后重试。`);
