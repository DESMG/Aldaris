    ...
    return <Dialog open onClose={close} fullWidth maxWidth="sm" aria-labelledby="create-issue-title">
        <Box component="form" onSubmit={async event => {
            ...
            if (!submission.current || submission.current.title !== issueTitle || submission.current.description !== text || submission.current.images.length !== images.length || images.some((file, index) => file !== submission.current!.images[index])) {
                submission.current = { title: issueTitle, description: text, images, key: crypto.randomUUID() };
            }
            ...
            <DialogContent>
                <Stack spacing={2} sx={{ pt: 1 }}>
                    ...
                    <TextField label="标题" required autoFocus fullWidth disabled={saving} slotProps={{ htmlInput: { maxLength: TITLE_MAX_LENGTH } }} value={title} onChange={event => { submission.current = null; setTitle(event.target.value); }} />
                    <Description label="描述" value={description} onChange={value => { submission.current = null; setDescription(value); }} images={images} onImagesChange={files => { submission.current = null; setImages(files); }} disabled={saving} onProcessingChange={setProcessing} />
