...
const labels: Record<OperationEvent["action"], string> = {
    setup: "初始化管理员", user_created: "创建账户", user_updated: "编辑账户", user_deleted: "删除账户", password_reset: "重置密码",
...
export default function OperationEvents({ user }: { user: User }) {
    ...
    return <Stack spacing={2}>
        <Box><Button href="/">返回列表</Button></Box>
        <Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "center" }}>
            <Typography variant="h5" component="h1">操作记录</Typography>
            <Button disabled={loading} onClick={() => { setBefore(null); setRefresh(value => value + 1); }}>刷新</Button>
        </Stack>
